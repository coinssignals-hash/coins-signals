...
function calcRiskMetrics(trades: ReturnType<typeof useImportedTrades>['trades']) {
  ...
  closed.forEach(t => {
    ...
    s.count++; s.pnl += t.net_profit; if (t.net_profit > 0) s.wins++;
  ...
  return { totalReturn, stdDev, var95, maxDDValue, maxDDPercent, recoveryFactor, calmarRatio, rorSimple, equityCurve, topSymbols, totalTrades: closed.length };
}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{
            background: 'linear-gradient(165deg, hsl(0 70% 55% / 0.25), hsl(0 70% 55% / 0.08))',
            border: '1px solid hsl(0 70% 55% / 0.3)',
          }}>
            <ShieldAlert className="w-4 h-4" style={{ color: 'hsl(0 70% 55%)' }} />
          </div>
          <div>
            <h3 className="text-sm font-bold text-foreground">Dashboard de Riesgo</h3>
            <p className="text-[10px] text-muted-foreground">Análisis de exposición y riesgo</p>
          </div>
        </div>
        <Card className="bg-card border-border">
    ...
    <div className="space-y-3">
          ...
          <div className="h-44">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={metrics.equityCurve}>
                <defs>
                  <linearGradient id="eqGradPerf" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="monotone" dataKey={showDD ? 'drawdown' : 'equity'} stroke={showDD ? 'hsl(0, 72%, 51%)' : 'hsl(var(--primary))'} fill="url(#eqGradPerf)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
      ...
      <Card className="bg-card border-border">
        <CardContent className="p-3 space-y-2">
          ...
          <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Desviación Estándar</span><span className="text-xs font-bold text-foreground">${metrics.stdDev.toFixed(2)}</span></div>
          <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Calmar Ratio</span><span className="text-xs font-bold text-foreground">{metrics.calmarRatio.toFixed(3)}</span></div>
          <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Retorno Total</span><span className={cn("text-xs font-bold", metrics.totalReturn >= 0 ? 'text-emerald-400' : 'text-red-400')}>{metrics.totalReturn >= 0 ? '+' : ''}${metrics.totalReturn.toFixed(2)}</span></div>
        </CardContent>
      </Card>
            ...
            {metrics.topSymbols.map(s => (
              <div key={s.symbol} className="space-y-1">
                ...
                <div className="h-1.5 rounded-full bg-muted/40 overflow-hidden">
                  <div className={cn("h-full rounded-full", s.concentration > 40 ? 'bg-amber-500' : 'bg-primary')} style={{ width: `${s.concentration}%` }} />
                </div>
                {s.concentration > 40 && (
