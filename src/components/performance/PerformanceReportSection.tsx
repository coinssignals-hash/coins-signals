...
function getWeeklyPnL(trades: ImportedTrade[], month: Date) {
  ...
  days.forEach((day, i) => {
    ...
    if (getDay(day) === 0 || i === days.length - 1) {
      if (currentWeek.trades > 0 || i === days.length - 1) weeks.push({ ...currentWeek });
...
function calcAdvancedStats(trades: ImportedTrade[]) {
  ...
  return {
    totalReturn, winRate: winRate * 100, sharpe, maxDD, maxConsecWins, maxConsecLosses,
    expectancy, totalTrades: closed.length, avgWin, avgLoss,
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{
          background: 'linear-gradient(165deg, hsl(30 80% 55% / 0.25), hsl(30 80% 55% / 0.08))',
          border: '1px solid hsl(30 80% 55% / 0.3)',
        }}>
          <Calendar className="w-4 h-4" style={{ color: 'hsl(30 80% 55%)' }} />
        </div>
        <div>
          <h3 className="text-sm font-bold text-foreground">Reporte de Rendimiento</h3>
          <p className="text-[10px] text-muted-foreground">Análisis mensual de operaciones</p>
        </div>
      </div>
      ...
      {viewMode === 'monthly' && (
        <Card className="bg-card border-border">
          <CardContent className="p-3">
            <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">P&L Diario — Heatmap</span>
            ...
            <div className="flex items-center justify-center gap-3 mt-3">
              <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-red-500/40" /><span className="text-[8px] text-muted-foreground">Pérdida</span></div>
              <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-muted/30" /><span className="text-[8px] text-muted-foreground">Sin trades</span></div>
              <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-emerald-500/40" /><span className="text-[8px] text-muted-foreground">Ganancia</span></div>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          {weeklyData.map(week => (
            <Card key={week.weekNum} className="bg-card border-border">
              <CardContent className="p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Semana {week.weekNum}</span>
                    <p className="text-xs text-muted-foreground mt-0.5">{week.trades} trades · WR {week.trades > 0 ? ((week.wins / week.trades) * 100).toFixed(0) : 0}%</p>
                  </div>
                ...
                <div className="h-1.5 rounded-full bg-muted/40 mt-2 overflow-hidden">
                  <div className={cn("h-full rounded-full transition-all", week.pnl >= 0 ? 'bg-emerald-500' : 'bg-red-500')} style={{ width: `${week.trades > 0 ? (week.wins / week.trades) * 100 : 0}%` }} />
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      {stats && (
        <>
          <h3 className="text-xs font-semibold text-primary flex items-center gap-1.5 mt-1">
          ...
          <Card className="bg-card border-border">
            <CardContent className="p-3 space-y-2">
              <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Resumen del Mes</span>
              <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Total Trades</span><span className="text-xs font-bold text-foreground">{stats.totalTrades}</span></div>
              <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Win Rate</span><span className="text-xs font-bold text-foreground">{stats.winRate.toFixed(1)}%</span></div>
              <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Retorno Total</span><span className={cn("text-xs font-bold", stats.totalReturn >= 0 ? 'text-emerald-400' : 'text-red-400')}>{stats.totalReturn >= 0 ? '+' : ''}${stats.totalReturn.toFixed(2)}</span></div>
              <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Avg Win / Loss</span><span className="text-xs font-bold text-foreground">${stats.avgWin.toFixed(2)} / ${stats.avgLoss.toFixed(2)}</span></div>
            </CardContent>
          </Card>
        </>
      )}
