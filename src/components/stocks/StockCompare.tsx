...
function SymbolPicker({ label, color, selected, onSelect, onClear }: {
  label: string; color: string; selected: string; onSelect: (s: string) => void; onClear: () => void;
}) {
        ...
        <div className="absolute z-50 mt-1 w-full bg-popover border border-border rounded-lg shadow-xl max-h-40 overflow-y-auto">
          {isLoading ? (
            ...
            results.slice(0, 5).map((r) => (
              <button
                key={r.symbol}
                onClick={() => { onSelect(r.symbol); setQuery(''); }}
    ...
    <div className="px-4 space-y-3 pb-4">
      ...
      <Card className="p-3 bg-card border-border space-y-2">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Comparar Acciones</span>
          <button onClick={onClose} className="ml-auto text-xs text-muted-foreground hover:text-foreground">Cerrar</button>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <SymbolPicker label="Acción A" color={COLOR_A} selected={symbolA} onSelect={setSymbolA} onClear={() => setSymbolA('')} />
          <SymbolPicker label="Acción B" color={COLOR_B} selected={symbolB} onSelect={setSymbolB} onClear={() => setSymbolB('')} />
        </div>
      </Card>
      ...
      {bothSelected && (
        <Card className="p-4 bg-card border-border">
          <h3 className="text-sm font-semibold text-foreground mb-3">Rendimiento relativo (3M)</h3>
          {loadA || loadB ? (
      ...
      {bothSelected && quoteA && quoteB && (
        <Card className="p-4 bg-card border-border">
          <div className="grid grid-cols-3 items-center mb-3">
            ...
            <span className="text-[10px] text-muted-foreground text-center uppercase font-semibold">Métrica</span>
          ...
          <MetricRow label="Precio" valueA={quoteA.price} valueB={quoteB.price} format="currency" />
          <MetricRow label="Cambio %" valueA={quoteA.changesPercentage} valueB={quoteB.changesPercentage} format="percent" />
          ...
          <MetricRow label="Volumen" valueA={quoteA.volume} valueB={quoteB.volume} format="large" />
          <MetricRow label="Máx. Día" valueA={quoteA.dayHigh} valueB={quoteB.dayHigh} format="currency" />
          <MetricRow label="Mín. Día" valueA={quoteA.dayLow} valueB={quoteB.dayLow} format="currency" higherIsBetter={false} />
          <MetricRow label="Máx. 52s" valueA={quoteA.yearHigh} valueB={quoteB.yearHigh} format="currency" />
          <MetricRow label="Mín. 52s" valueA={quoteA.yearLow} valueB={quoteB.yearLow} format="currency" higherIsBetter={false} />
        </Card>
      )}

      {!bothSelected && (
        <Card className="p-6 bg-card border-border">
          <p className="text-sm text-muted-foreground text-center">Selecciona dos acciones para comparar sus métricas y rendimiento</p>
        </Card>
      )}
    </div>
  );
