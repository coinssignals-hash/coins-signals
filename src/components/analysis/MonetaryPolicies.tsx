  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className="rounded-xl overflow-hidden border border-cyan-800/30">
      <CollapsibleTrigger className="w-full bg-slate-800/60 px-4 py-3 flex items-center justify-between hover:bg-slate-700/60 transition-colors">
        <h3 className="text-white font-semibold text-sm">{t('analysis_monetary_policies')}</h3>
        ...
        <div className="p-4 border-t border-cyan-800/30">
          {isLoading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-gray-400">{t('analysis_loading_policies')}</span>
            </div>
          ) : error || !policies ? (
            <AnalysisError 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {policies.map((policy, index) => (
                <div key={index} className="space-y-3 border border-slate-700/30 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-400">{t('analysis_current_rate')}:</span>
                      <span className="text-cyan-400">{policy.currentRate}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">{t('analysis_last_decision')}:</span>
                      <span className="text-cyan-400 text-xs">{policy.lastDecision}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">{t('analysis_next_meeting')}:</span>
                      <span className="text-cyan-400">{policy.nextMeeting}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">{t('analysis_expectations')}:</span>
                      <span className="text-cyan-400 text-xs">{policy.expectations}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">{t('analysis_end_year_rate')}:</span>
