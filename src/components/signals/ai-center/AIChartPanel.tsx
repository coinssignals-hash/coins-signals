        ...
        <Collapsible>
          <CollapsibleTrigger className="flex items-center justify-between w-full px-3 py-2 rounded-lg bg-card border border-border hover:bg-secondary/50 transition-colors group">
            <div className="flex items-center gap-2">
              ...
              {bullishPatterns.length > 0 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-bullish/10 text-bullish font-medium flex items-center gap-0.5">
                  <TrendingUp className="w-3 h-3" />{bullishPatterns.length}
                </span>
              )}
              {bearishPatterns.length > 0 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-destructive/10 text-destructive font-medium flex items-center gap-0.5">
                  <TrendingDown className="w-3 h-3" />{bearishPatterns.length}
                </span>
              )}
            </div>
          ...
          <CollapsibleContent className="mt-1.5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
              {[...bullishPatterns, ...bearishPatterns, ...neutralPatterns].map((p, i) => {
                const Icon = p.type === 'bullish' ? TrendingUp : p.type === 'bearish' ? TrendingDown : Minus;
                const colorClass = p.type === 'bullish' ? 'text-bullish' : p.type === 'bearish' ? 'text-destructive' : 'text-accent';
                return (
                  <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-md bg-card/50 border border-border/50">
                    <Icon className={cn("w-3 h-3 flex-shrink-0", colorClass)} />
                    <span className="text-[11px] font-medium text-foreground truncate flex-1">{p.name}</span>
                    <span className={cn(
                      "text-[9px] px-1 py-0.5 rounded font-medium",
                      p.reliability === 'high' ? 'bg-primary/10 text-primary' :
