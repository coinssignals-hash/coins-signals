      ...
      <main className="container py-3 max-w-lg mx-auto px-3 space-y-3">
            ...
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-foreground flex items-center gap-1.5">
                  {t('gl_title')} <Sparkles className="w-4 h-4" style={{ color: 'hsl(40 80% 55%)' }} />
                </h1>
                <p className="text-xs text-muted-foreground">{glossaryTerms.length} {t('gl_essential_terms')}</p>
              </div>
            </div>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder={t('gl_search')}
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
          <button onClick={() => setActiveCategory(null)}
            ...
            style={{ background: !activeCategory ? `hsl(${color} / 0.15)` : 'hsl(var(--card) / 0.5)', border: `1px solid ${!activeCategory ? `hsl(${color} / 0.3)` : 'hsl(var(--border) / 0.3)'}`, color: !activeCategory ? `hsl(${color})` : undefined }}>
            {t('gl_all')}
          </button>
          {glossaryCategories.map(cat => (
            <button key={cat.id} onClick={() => setActiveCategory(activeCategory === cat.id ? null : cat.id)}
              ...
              style={{ background: activeCategory === cat.id ? `hsl(${cat.color} / 0.15)` : 'hsl(var(--card) / 0.5)', border: `1px solid ${activeCategory === cat.id ? `hsl(${cat.color} / 0.3)` : 'hsl(var(--border) / 0.3)'}`, color: activeCategory === cat.id ? `hsl(${cat.color})` : undefined }}>
              {cat.name}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {filtered.length === 0 && (
            <div className="text-center py-8 text-muted-foreground text-sm">{t('gl_no_results')}</div>
          )}
          {filtered.map((term, i) => {
            ...
            return (
              <motion.div key={term.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.03, 0.3) }}>
                <button onClick={() => setExpandedTerm(isExpanded ? null : term.id)} className="w-full text-left">
                  <GlowSection color={termColor} style={{ borderColor: isExpanded ? `hsl(${termColor} / 0.3)` : `hsl(${termColor} / 0.12)` }}>
                      ...
                      {isExpanded && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                          <div className="px-3 pb-3 space-y-2">
                            ...
                            {term.example && (
                              <div className="p-2.5 rounded-lg text-xs" style={{ background: `hsl(${termColor} / 0.06)`, border: `1px solid hsl(${termColor} / 0.15)` }}>
                                <span className="font-semibold" style={{ color: `hsl(${termColor})` }}>{t('gl_example')}: </span>
                                <span className="text-muted-foreground">{term.example}</span>
                              </div>
                            )}
                            <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ background: `hsl(${termColor} / 0.1)`, color: `hsl(${termColor})`, border: `1px solid hsl(${termColor} / 0.2)` }}>
                              {cat?.name}
                            </span>
                          </div>
                        </motion.div>
