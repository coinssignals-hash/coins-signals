        ...
        {!loading && !error && (
          <div className="space-y-4">
            {filteredBrokers.map((broker, idx) => {
              ...
              const ratingPct = (broker.rating / 5) * 100;
              const ratingColor = broker.rating >= 4.5 ? 'hsl(var(--bullish))' : broker.rating >= 4 ? 'hsl(var(--accent))' : 'hsl(var(--muted-foreground))';
              const logo = getBrokerLogo(broker.name);
              return (
                <motion.div
                  key={broker.id}
                ...
                >
                  <Card
                    className={`overflow-hidden transition-all active:scale-[0.98] cursor-pointer group ${isSelectedForCompare ? 'ring-2 ring-primary shadow-md' : ''}`}
                    ...
                    <CardContent className="p-0">
                      ...
                      <div className="relative">
                        <div className="absolute inset-0 opacity-[0.06]" style={{
                          background: `linear-gradient(135deg, ${ratingColor}, transparent 70%)`,
                        }} />
                        <div className="relative flex items-center gap-3 p-3.5 pb-3">
                          {compareMode && (
                            <Checkbox
                          ...
                          {/* Logo with glow */}
                          <div className="relative">
                            <div className="w-13 h-13 bg-secondary rounded-2xl flex items-center justify-center shrink-0 overflow-hidden border border-border/50 transition-transform duration-300 group-hover:scale-105"
                              style={{ width: 52, height: 52 }}
                            >
                              {logo ? (
                                <LazyImage src={logo} alt={broker.name} className="w-full h-full object-contain p-1.5" />
                              ) : (
                                <span className="text-2xl">{broker.countryFlag}</span>
                              )}
                            </div>
                            {broker.rating >= 4.5 && (
                              <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-amber-400/90 flex items-center justify-center shadow-sm">
                                <Star className="w-3 h-3 fill-white text-white" />
                              </div>
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                            <h3 className="text-[15px] font-bold text-foreground truncate leading-tight">{broker.name}</h3>
                            ...
                            <div className="flex items-center gap-2 mt-1.5">
                              <div className="flex-1 h-1.5 rounded-full bg-secondary overflow-hidden">
                                <motion.div
                                  ...
                                  transition={{ duration: 0.8, delay: idx * 0.06 + 0.3, ease: 'easeOut' }}
                                />
                              </div>
                              <span className="text-xs font-bold tabular-nums" style={{ color: ratingColor }}>{broker.rating}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Stats grid — enhanced */}
                      <div className="grid grid-cols-3 gap-px bg-border/40 mx-3 rounded-xl overflow-hidden mb-2.5">
                        <div className="bg-card/80 px-2 py-2.5 text-center">
                          <Coins className="w-3.5 h-3.5 mx-auto mb-1 text-primary opacity-60" />
                          <span className="text-[8px] uppercase tracking-widest text-muted-foreground block">{t('broker_initial_deposit')}</span>
                          <span className="text-xs font-bold text-foreground mt-0.5 block">{broker.depositMin}</span>
                        </div>
                        <div className="bg-card/80 px-2 py-2.5 text-center">
                          <TrendingUp className="w-3.5 h-3.5 mx-auto mb-1 text-primary opacity-60" />
                          <span className="text-[8px] uppercase tracking-widest text-muted-foreground block">{t('broker_spreads')}</span>
                          <span className="text-xs font-bold text-foreground mt-0.5 block truncate">{broker.spreads || 'Variable'}</span>
                        </div>
                        <div className="bg-card/80 px-2 py-2.5 text-center">
                          <BarChart3 className="w-3.5 h-3.5 mx-auto mb-1 text-primary opacity-60" />
                          <span className="text-[8px] uppercase tracking-widest text-muted-foreground block">{t('broker_leverage')}</span>
                          <span className="text-xs font-bold text-foreground mt-0.5 block truncate">{broker.leverage || 'N/A'}</span>
                        </div>
                      </div>
                      ...
                      {/* Regulation & Instruments tags */}
                      <div className="px-3 pb-2.5">
                        <div className="flex flex-wrap gap-1">
                          {broker.regulations.slice(0, 3).map(r => (
                            <span key={r} className="inline-flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded-md bg-primary/10 text-primary font-medium">
                      ...
                      {/* Footer */}
                      <div className="flex items-center justify-between px-3.5 py-2.5 border-t border-border/40 bg-secondary/20">
                        <span className="text-[10px] text-muted-foreground truncate max-w-[50%]">{broker.type}</span>
                        <div className="flex gap-1.5" onClick={(e) => e.stopPropagation()}>
                          ...
                          <Button size="sm" className="bg-primary hover:bg-primary/90 text-[10px] h-7 px-3 shadow-sm" onClick={() => setSelectedBroker(broker)}>
