...
import {
  ...
  LineChart, Gauge, PieChart, Workflow, Shield, Zap, Lock, ArrowUpDown, Star
} from 'lucide-react';
...
function FeaturedCarousel({ tools, navigate, t }: { tools: ToolItem[]; navigate: (path: string) => void; t: (k: string) => string }) {
        ...
        {top4.map((tool, i) => {
          ...
          return (
            <motion.button
              ...
              className="snap-start shrink-0 w-[160px] rounded-xl p-3 flex flex-col items-start gap-2 text-left transition-all active:scale-95"
              style={{
      ...
      <div className="flex items-center justify-center gap-1 mt-1">
        {top4.map((tool, i) => (
          <div
            ...
            style={{ background: `hsl(${tool.color} / 0.4)` }}
          />
        ))}
      </div>
    </div>
  ...
  return (
    <PageShell>
      ...
      <main className="container py-3 max-w-lg mx-auto px-3">
        {/* Featured Carousel — swipeable cards */}
        <FeaturedCarousel tools={ALL_TOOLS} navigate={navigate} t={t} />
