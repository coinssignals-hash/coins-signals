...
@cached("news:date", ttl=300)  # 5 min
...
@cached("news:currency", ttl=300)  # 5 min
...
@cached("news:trends", ttl=3600)  # 1 hour
async def get_news_trends(year: int, month: int):
    """Get historical news trends aggregated by month/year"""
...
@cached("news:trends_range", ttl=3600)  # 1 hour
async def get_news_trends_range(
    ...
    """Get historical news trends for a date range"""
    results = []
...
@cached("news:top_impact", ttl=300)  # 5 min
async def get_top_news_by_impact(
    ...
    """Get top news ranked by currency impact score"""
    scored_news = []
